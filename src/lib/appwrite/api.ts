...
export async function createUserAccount(user: INewUser) {
  try {
    ...
    // 2. ذخیره اطلاعات تکمیلی در دیتابیس
...
export async function signInAccount(user: { email: string; password: string }) {
  ...
  try {
    ...
    // حذف همه sessionها به جز session جاری
  ...
  try {
    ...
    const currentUser = await databases.listDocuments(
      ...
      [Query.equal("accountId", currentAccount.$id)]
        ...
        // اگر کاربر در دیتابیس نیست، ایجادش کن
    if (currentUser.documents.length === 0) {
      ...
      const currentUser = await databases.createDocument(
        ...
        ID.unique(),
        {
          ...
          username: currentAccount.email.split('@')[0], // یا یک username پیش‌فرض
          imageUrl: avatarUrl
        }
      );
...
export function getFileView(fileId: string) {
  try {
    ...
    if (!fileUrl) throw Error;
    console.log("getFileView =>", fileUrl);
    return fileUrl;
  } catch (error) {
    console.log(error);
    return null;
...
export async function getInfinitePosts({ pageParam }: { pageParam: string | null }) {
  ...
  try {
    ...
    throw error; // خطا را throw کنید تا React Query بتواند آن را مدیریت کند
  }
}
